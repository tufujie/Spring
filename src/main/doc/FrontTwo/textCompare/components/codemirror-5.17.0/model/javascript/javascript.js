...
function objprop(type,value){if(type=="async")return cont(objprop);if(type=="variable"||cx.style=="keyword"){cx.marked="property";if(value=="get"||value=="set")return cont(getterSetter);return cont(afterprop);}else if(type=="number"||type=="string"){cx.marked=jsonldMode?"property":(cx.style+" property");return cont(afterprop);}else if(type=="jsonld-keyword"){return cont(afterprop);}else if(type=="modifier"){return cont(objprop)}else if(type=="["){return cont(expression,expect("]"),afterprop);}else if(type=="spread"){return cont(expression);}}
...
function maybetype(type){if(isTS&&type==":")return cont(typeexpr);}
...
function arrayLiteral(type){if(type=="]")return cont();return pass(expressionNoComma,commasep(expressionNoComma,"]"));}
